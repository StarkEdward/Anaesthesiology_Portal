...
interface DoctorAttendanceReportTemplateProps {
  records: DoctorAttendanceRecord[];
  fromDate: string;
  toDate: string;
  reportDate: string;
  refYear: string;
...
export const DoctorAttendanceReportTemplate: React.FC<DoctorAttendanceReportTemplateProps> = ({
  records,
  fromDate,
  toDate,
  reportDate,
  ...
  const fs = fontSizes || { title: 30, subtitle: 20, english: 18.4, body: 14, table: 14, tablePadding: 5 };
  const tPad = fs.tablePadding !== undefined ? fs.tablePadding : 5;
    ...
    <div className="print-page relative bg-white text-black mx-auto flex flex-col font-sans" style={{ width: '210mm', minHeight: '297mm', padding: '15mm 20mm' }}>
      ...
      <div className="text-center mb-4 flex flex-col gap-1">
        ...
        <div className="border-t border-b border-black py-1 mt-1 flex justify-between font-bold" style={{ fontSize: Math.max(10, fs.body - 2) }}>
          <div>जा.क्र.शावैमनं/बधिरीकरणशास्त्र वि./अध्यापक उपस्थिती अहवाल/ <span className="inline-block w-16"></span> /{refYear.replace(/^\//, '')}</div>
      ...
      <div className="text-center font-bold mb-6 underline underline-offset-4" style={{ fontSize: fs.body + 1 }}>
        विषय :- बधिरीकरणशास्त्र विभागातील अध्यापकांचा उपस्थिती अहवाल सादर करणेबाबत...
      </div>

      <div className="mb-4" style={{ fontSize: fs.body }}>
        <div>मा. महोदय,</div>
        <div className="indent-8 text-justify mt-2 leading-relaxed">
          उपरोक्त विषयास अनुसरून बधिरीकरणशास्त्र विभाग, ज.बि.मुं. शासकीय वैद्यकीय महाविद्यालय, नंदुरबार येथे कार्यरत असलेल्या अध्यापकांचा दि. {fromDate} ते दि. {toDate} या कालावधीतील उपस्थिती बाबतचा अहवाल खालीलप्रमाणे सादर करण्यात येत आहे.
        </div>
      </div>
      ...
      <table className="w-full border-collapse border border-black text-center mb-8" style={{ fontSize: fs.table }}>
        ...
        <tbody>
          {records.map((doc, idx) => (
            <tr key={doc.id}>
              <td className="border border-black font-bold" style={{ padding: tPad }}>{idx + 1}</td>
